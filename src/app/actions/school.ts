...
export async function createSchoolCourseAction(
  ...
  const slug = formData.get("platformCourseSlug") as string;

  const parsed = createSchoolCourseSchema.safeParse({
    title: formData.get("title"),
    description: formData.get("description") || undefined,
    platformCourseSlug: slug && slug !== "" ? slug : undefined,
  });
...
export async function enrollStudentAction(
  ...
  const parsed = enrollStudentSchema.safeParse({ userId: formData.get("userId") });
  if (!parsed.success) {
    return { success: false, error: parsed.error.issues[0]?.message ?? "Dados inválidos" };
  }
...
export async function assignTeacherAction(
  ...
  const parsed = assignTeacherSchema.safeParse({ userId: formData.get("userId") });
  if (!parsed.success) {
    return { success: false, error: parsed.error.issues[0]?.message ?? "Dados inválidos" };
  }
